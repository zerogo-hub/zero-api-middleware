...
func createtoken(ctx zeroapi.Context) {
	...
	ctx.SetCookie(
		"token",
		token,
		zeroctx.WithCookieHTTPOnly(true),
		zeroctx.WithCookieMaxAge(30*24*3600),
	)

	ctx.Text("please visit http://127.0.0.1:8877/check")
}
...
func onToken(ctx zeroapi.Context) (string, error) {
	return zamjwt.TokenFromCookie(ctx, "token")
}
